...
| 会員登録 / ログイン | バリデーション付きフォーム・好きなジャンル複数選択・ログアウト確認ダイアログ |
| チェックアウト | 住所入力・配送希望日（テキスト入力＋📅カレンダー選択、翌日〜1ヶ月先）・配送希望時間帯選択・支払い方法選択・カードブランド自動判定（Visa/MC/AMEX/JCB） |
| 注文完了 | 注文番号発行（`GS-YYYYMMDD-HHmmss-XXXX` 形式） |
| 購入履歴 | 注文番号検索・アコーディオン展開・ページネーション（既存データも `GS-` 形式で統一表示） |
| 商品評価 | 購入済み商品への星評価＋コメント投稿（モーダル） |
| マイページ | ユーザー情報確認・好きなジャンル設定・デフォルトカード表示・最近の注文 |
| お気に入り | 商品のブックマーク・スケルトンUIローディング |
| お知らせバナー | 期間限定セール告知など |
| カルーセルバナー | 6枚バナーを3秒自動スライド・左右矢印・ドットナビ付き |
